...
module.exports = class LPoSChainModule {
	...
	async generateCandidacyToken(candidateAddress, stakingPassphrase) {
		...
		let candidateBalance = await this.dal.getWalletBalance(candidateAddress);
		return {
			candidateAddress,
			candidateBalance,
	...
	async load(channel, options) {
				...
				await Promise.all(
					candidacyList.map(async (candidacyToken) => {
						if (!this._candidacyMap[candidacyToken.candidateAddress]) {
							...
							try {
								candidateBalance = await this.dal.getWalletBalance(candidacyToken.candidateAddress);
							} catch (error) {
								this.logger.error(
		...
		this._candidacyListBroadcastIntervalId = setInterval(async () => {
			if (this._stakingPassphrase) {
				if (!this._candidacyMap[this._walletAddress]) {
					try {
						let candidacyToken = await this.generateCandidacyToken(this._walletAddress, this._stakingPassphrase);
